...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String recommendWhoToFollow(String name) {
        int maxMutual = 0;
        User toRecommend  = getUser(name);
        String recommendedName = "";
        for (int i = 0; i < userCount; i++){
            if (users[i].getName().equals(name)){
                continue;
            }
            int curMutual = users[i].countMutual(toRecommend);
            if(maxMutual < curMutual){
                maxMutual = curMutual;
                recommendedName = users[i].getName();
            }
    ...
    public String mostPopularUser() {
        ...
        for (int i = 0; i < userCount; i++){
            int curFollowee = followeeCount(users[i].getName());
            if (maxFollowee < curFollowee) {
                maxFollowee = curFollowee;
